...
def t():
  ...
  tk.Label(t1,text='爱好:',font=('楷体',16)).grid(row=3,column=1)
  ...
  #创建多选框
  tk.Checkbutton(t1,text='跑步',variable=s3,onvalue=1,offvalue=0).place(x=80,y=57)
  tk.Checkbutton(t1,text='游泳',variable=s4,onvalue=1,offvalue=0).place(x=130,y=57)
  def tj():
    print(t2.get())
    print(s2.get())
    if s3.get()==1:
